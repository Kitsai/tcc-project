...
- Directory structure inside each problem folder: `files/`, `solutions/`, `tests/main/`, `tests/validator/`, `tests/checker/`, `tests/export/`, `statement/`.
...
- **Export staging** (`export.rs`): resolves the problem's `TestDefinition`s (`tests/main/`, the actual judge test suite — not to be confused with the checker/validator meta-tests above) into a flat, numbered test suite under `tests/export/`. `prepare_export_dir` wipes and recreates that directory on every run (same disposable-artifact idiom as `bin/` — see `commands/dev.rs::clean_binaries`). The resolved suite is recorded as an `ExportManifest` (`tests/export/manifest.json`, path via `manifest_path`), mapping each final id back to its source `TestDefinition` id and `example` flag; the manifest is immutable once written (later validate/solve/check stages read it but never rewrite it — no per-test results are persisted, since verification is fail-fast: the first failing test aborts the whole run). Known limitation: a `Script` definition whose generator batches out multiple files in one run (testlib's `startTest` convention, e.g. a single generator producing an entire 100-test suite) has every expanded file inherit that one definition's `example` flag — there's no way yet to mark only some tests within such a batch as samples.
