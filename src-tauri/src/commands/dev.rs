...
use crate::{constants::BINARY_DIR, error::AppResult, problem::ProblemManager, util::ResultExt};
...
#[tauri::command]
pub fn clean_binaries(state: State<ProblemManager>) -> AppResult<()> {
    if !cfg!(debug_assertions) {
        return Err("clean_binaries is only available in debug builds".into());
    }
